async def stagnation_detector_node(state: GraphWeaveState, config: GraphWeaveConfig) -> GraphWeaveState:
    """
    Purpose: Detect repeated identical routing decisions
    Inputs: state.stagnation_history
    Outputs: state (mutates) or raises StagnationError
    Failure: FORCE_EXIT with stagnation flag
    """
    if len(state["stagnation_history"]) < 3:
        return state

    # Compare last 3 routing directives (ignore timestamps, only intent)
...
async def stagnation_detector(state: OrchestratorState, config: RunnableConfig) -> Dict:
    ...
    if repeat_count >= threshold:
        notice = SystemMessage(content=f"STAGNATION DETECTED: Aborted after {repeat_count} identical requests.")
        return {
