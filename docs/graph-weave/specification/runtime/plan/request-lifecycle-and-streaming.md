...
- thread_id is generated by the gateway and preserved through submission, execution, and streaming.

## Why Two IDs Exist

- `run_id` stays stable as the public job record.
- `thread_id` can represent a live attempt inside that job.
- This leaves room for retries, reruns, or recovery flows where the same run may later attach to a new live execution.
