      ...
      <Card glass className="border-none animate-pulse">
        <CardContent className="p-6 flex items-center justify-center gap-3">
          ...
          <span className="text-sm font-medium text-muted-foreground italic">
            AI Analyst is calculating your daily briefing...
          </span>
        </CardContent>
      </Card>
    );
      ...
      {data?.alerts && data.alerts.length > 0 && (
        <>
          ...
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold uppercase tracking-wider text-primary flex items-center gap-2">Alerts</h3>
          </div>
            ...
            {data.alerts.map((alert, i) => (
              <div
                ...
                className={`flex-none w-[280px] p-4 rounded-xl border flex gap-3 transition-all hover:scale-[1.02] cursor-default ${
                  alert.type === 'critical'
                    ? 'bg-rose-50 border-rose-100 text-rose-900 dark:bg-rose-950/20 dark:border-rose-900/50 dark:text-rose-200'
                    : alert.type === 'warning'
                      ? 'bg-amber-50 border-amber-100 text-amber-900 dark:bg-amber-950/20 dark:border-amber-900/50 dark:text-amber-200'
