...
import { ArrowRight, ArrowLeft, Check, Sparkles, Wallet, Calendar } from "lucide-react";
  ...
  return (
    <div className="max-w-xl mx-auto py-12">
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
             ...
             <div className="grid grid-cols-2 gap-4">
               {CATEGORIES.map((c) => (
                 <button
                   key={c.type}
                   onClick={() => { updateData("type", c.type); updateData("name", c.label); }}
                   className={cn(
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
             ...
             <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground ml-1">Target Amount (₫)</label>
                  <Input 
                    type="number" 
                    placeholder="0"
                ...
                <div className="space-y-2 text-center p-4 bg-indigo-500/5 rounded-2xl border border-indigo-500/10">
                   ...
                   <p className="text-xs text-muted-foreground">
                     Based on your savings history, a target of <span className="text-foreground font-bold italic">₫50M</span> within 12 months is highly achievable.
                   </p>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground ml-1">Deadline Date</label>
                ...
                {["MANUAL", "AUTOMATIC", "AI-MANAGED"].map((type) => (
                  <button
                    ...
                    className={cn(
                      ...
                      data.contributionType === type ? "border-indigo-600 bg-indigo-50/50" : "border-muted hover:border-indigo-200"
                    )}
                  >
                    <div>
                    ...
                    {data.contributionType === type && <div className="h-6 w-6 rounded-full bg-indigo-600 flex items-center justify-center text-white"><Check size={14} strokeWidth={4} /></div>}
                  </button>
                ))}
             </div>
          </div>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 text-center">
             <div className="flex justify-center flex-col items-center gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground">Your <span className="text-foreground font-bold italic">{data.name}</span> goal is ready to launch.</p>
                </div>
             </div>
      ...
      <div className="mt-12 flex justify-between gap-4">
        {step > 1 && step < 4 && (
          <Button variant="ghost" className="h-14 px-8 rounded-2xl font-bold uppercase tracking-widest text-xs gap-2" onClick={prev}>
            <ArrowLeft className="h-4 w-4" />
            Back
          </Button>
        )}
        {step < 4 ? (
          <Button className="h-14 px-8 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white ml-auto font-black uppercase tracking-widest text-xs gap-2" onClick={next}>
            Continue
            <ArrowRight className="h-4 w-4" />
          </Button>
        ) : (
          <Button className="h-16 w-full rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase tracking-widest text-lg shadow-xl shadow-indigo-600/20" onClick={() => window.location.href = '/accounts/goals'}>
            Launch Goal
          </Button>
        )}
      </div>
