...
class TestDevOpsLogAnalyzerE2E:
    ...
    def test_incident_summary_available_within_latency_window(
        ...
        Then: The final report should be available within the expected latency window (< 120 seconds)
        """
        debug_log(
        ...
        assert final is not None
        assert final["status"] in ["completed", "failed"]
        assert elapsed < 120.0, f"Incident analysis took {elapsed:.2f}s, exceeds 120.0s SLA"
