...
class TestCheckpointRecoveryE2E:
    def test_recover_missing_run_returns_404(self, client):
        response = client.post(
            "/execute/missing-run/recover", json={"thread_id": "thread-1"}
        )
    ...
    def test_recovery_requires_checkpoint(self, client):
        response = client.post("/execute/run-1/recover", json={"thread_id": "thread-1"})
        assert response.status_code == 404
