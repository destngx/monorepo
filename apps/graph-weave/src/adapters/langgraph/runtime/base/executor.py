...
from ..state import StateResolver, StateContext
...
class BaseLangGraphExecutor:
    ...
    def _get_state_value(self, path: Any, state: Mapping[str, Any]) -> Any:
        ...
        if isinstance(path, str) and path.startswith("$."):
            try:
                return StateResolver(state).resolve(path, required=False)
            except Exception:
                pass
    ...
    def _state_context(self, state: Mapping[str, Any], node_input_contract: Optional[Any] = None) -> StateContext:
        return StateContext(state, node_input_contract=node_input_contract)
