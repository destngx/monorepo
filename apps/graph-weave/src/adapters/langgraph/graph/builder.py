...
class GraphBuilder:
    ...
    def _build_from_parsed(self, parsed: Dict[str, Any], workflow_dict: Dict[str, Any]) -> Dict[str, Any]:
        nodes_by_id = {}
        for node_dict in parsed["nodes"]:
            ...
            if node_type == "entry":
                ...
                nodes_by_id[node_id] = build_agent_node(config, system_prompt, user_template)
    ...
    async def build(self, workflow_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return self._build_from_parsed(parsed, workflow_dict)

    @staticmethod
    def build_sync(workflow_dict: Dict[str, Any]) -> Dict[str, Any]:
        parsed = WorkflowParser.parse_workflow_json(workflow_dict)
        builder = GraphBuilder()
        return builder._build_from_parsed(parsed, workflow_dict)
