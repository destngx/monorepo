              ...
              <div key={bank.name} className="space-y-6">
                ...
                <div className="grid grid-cols-1  gap-6">
                  {bank.accounts.map((account: BankAccountSummary) => (
                    <Card
                      key={account.name}
                      className="relative overflow-hidden border-primary/10 shadow-lg bg-card/50 backdrop-blur-sm"
                    >
                      ...
                      <CardContent className="pt-6 space-y-8">
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between text-sm font-medium">
                            ...
                            <span className="text-emerald-600">
                              Available: <MaskedBalance amount={account.remainingLimit} />
                            </span>
                          </div>
                        ...
                        <div className="grid grid-cols-3 gap-2 pb-2">
                          <div className="p-2 rounded-lg bg-emerald-500/5 border border-emerald-500/10 text-center">
                            ...
                            <p className="text-sm font-bold text-emerald-600 tracking-tighter">
                              <MaskedBalance amount={account.totalRefund} />
                            </p>
                          </div>
                          <div className="p-2 rounded-lg bg-orange-500/5 border border-orange-500/10 text-center">
                            ...
                            <p className="text-sm font-bold text-orange-600 tracking-tighter">
                              -<MaskedBalance amount={account.totalFees} />
                            </p>
                          </div>
                          <div className="p-2 rounded-lg bg-primary/5 border border-primary/10 text-center">
                            ...
                            <p className="text-sm font-bold text-primary tracking-tighter">
                              <MaskedBalance amount={account.totalEarn} />
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          ...
                          {account.cards.map((card: CardStat) => (
                            <div
                              key={card.name}
                              className={`p-4 rounded-xl border-l-4 shadow-sm transition-all hover:translate-x-1 duration-200 ${card.name.includes('UNIQ') ? 'border-emerald-500 bg-emerald-500/5' : 'border-blue-500 bg-blue-500/5'}`}
                            >
                              <div className="flex justify-between items-center mb-4">
                                <h4 className="font-bold text-sm flex items-center gap-2">
                                  <div
                                    className={`w-2 h-2 rounded-full ${card.name.includes('UNIQ') ? 'bg-emerald-500' : 'bg-blue-500'}`}
                                  />
                                  {card.name.replace('Sacombank Visa ', '')}
                                </h4>
                                <div className="text-right">
                                  <div className="text-xs font-medium text-emerald-600">
                                    <MaskedBalance amount={card.estimatedCashback} /> est. cashback
                                  </div>
                                  {card.expiry && (
          ...
          <div className="flex items-start gap-2 bg-blue-500/5 text-blue-800 dark:text-blue-300 p-3 flex-row rounded-lg border border-blue-500/20 text-xs">
            ...
            <div className="space-y-1">
              ...
              <p className="opacity-90">
                By default, metrics use the standard transaction date. However, for delayed{' '}
                <strong>Cashback Refunds</strong>, the system reads the <strong>Memo</strong> field to find the original
                target month (e.g., &quot;12/2025&quot;). The refund is then retroactively applied to that{' '}
                <em>Specific Month</em> across all charts, ensuring accurate performance tracking.
              </p>
            </div>
          </div>
                  ...
                  <tbody className="divide-y divide-border">
                    {allCardStats.map((card: CardStat) =>
                      card.history.map((h: CardHistory, i: number) => (
                        <tr
                          key={`${card.name}-${h.month}`}
                          className="bg-background hover:bg-muted/30 transition-colors"
                        >
                          <td className="px-6 py-4 font-medium">
                            {i === 0 && (
                              <span className="block text-xs font-bold text-primary mb-1 uppercase tracking-tighter">
                                {card.name}
                              </span>
                            )}
                          ...
                          <td className="px-6 py-4 text-right tabular-nums">
                            <MaskedBalance amount={h.spent} />
                          </td>
                          <td className="px-6 py-4 text-right tabular-nums text-muted-foreground">
                            <MaskedBalance amount={h.cashback} />
                          </td>
                          <td className="px-6 py-4 text-right tabular-nums text-emerald-600 font-bold">
                            {h.actualRefund > 0 ? (
                              <>
                                <span className="mr-0.5">+</span>
                                <MaskedBalance amount={h.actualRefund} />
                              </>
                            ) : (
                              '-'
                            )}
              ...
              <Card key={cardName} className="border-primary/5 shadow-md">
                <CardHeader className={`${cardName.includes('UNIQ') ? 'bg-emerald-500/5' : 'bg-blue-500/5'}`}>
                  <CardTitle className="text-base flex items-center gap-2">
                    <ShieldCheck
                      className={`h-5 w-5 ${cardName.includes('UNIQ') ? 'text-emerald-500' : 'text-blue-500'}`}
                    />
                ...
                <CardContent className="pt-6 space-y-4">
                  {rules.map((rule) => (
                    <div
                      key={rule.tag}
                      className={`flex justify-between items-center p-3 rounded-lg border ${rule.tag.includes('online') || rule.tag.includes('supermarket') ? 'bg-primary/5 border-primary/20' : ''}`}
                    >
                      ...
                      <span className={`font-bold ${rule.rate > 1 ? 'text-emerald-600' : 'text-muted-foreground'}`}>
                        {rule.rate}%
                      </span>
                    </div>
                  ))}
                  <div className="p-4 rounded-lg bg-muted/30 space-y-2 text-xs text-muted-foreground">
                    <div className="flex items-center gap-2 font-medium text-foreground italic underline">
                      Usage Policy
                    </div>
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${cardName.includes('UNIQ') ? 'bg-emerald-500' : 'bg-blue-500'}`}
                      />{' '}
                      Max <MaskedBalance amount={rules[0].capMonthly} /> cap for primary category
                    </div>
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${cardName.includes('UNIQ') ? 'bg-emerald-500' : 'bg-blue-500'}`}
                      />{' '}
                      Max <MaskedBalance amount={600000} /> total per cycle
                    </div>
                  </div>
                </CardContent>
              </Card>
