...
import { convertUSDTtoVND, formatVND } from '../utils/currency';
...
export const financialTools = {
  ...
  webSearch: tool({
    description: 'Search the web for real-time market data, financial news, crypto prices, interest rates, geopolitical events, or any external information needed to provide accurate advice. Use this proactively when the user asks about current events, prices, rates, or anything requiring up-to-date information.',
    inputSchema: z.object({
      query: z.string().describe('The search query — be specific and include relevant financial terms, asset names, dates, or regions'),
    }),
    execute: async ({ query }) => {
      const { executeSearch } = await import('@/lib/services/search-service');
      const { results, error } = await executeSearch(query);
      if (error && (!results || results.length === 0)) {
        return { results: [], error: `Search failed: ${error}. Inform the user and use your internal knowledge instead.` };
      }
      return {
