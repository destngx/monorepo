...
describe('Data Source Adapters', () => {
  ...
  describe('CafeFAdapter (last resort fallback)', () => {
  ...
  describe('YahooFinanceAdapter', () => {
    ...
    it('fetches US stock data successfully', async () => {
    ...
    it('returns data points with required fields', async () => {
      const result = await adapter.fetchHistorical('AAPL', 'US', '1d', '7d');
  ...
  describe('Adapter Chain Priority: VNStock → Yahoo → CafeF', () => {
    it('VN stocks: VNStock (if available) preferred over Yahoo', async () => {
      ...
      // At least one source should succeed in normal conditions
      expect(vnstockResult !== null || yahooResult !== null).toBe(true);
    }, 10000);

    it('CafeF is last resort: no historical for non-index symbols', async () => {
      const cafef = new CafeFAdapter();
      const result = await cafef.fetchHistorical('VCB', 'VN', '1d', '60d');
      expect(result).toBeNull();
    });
  });
});
