...
export function AIBudgetAdvisorView({ data }: { data?: AdvisorData }) {
  if (!data) return (
    <div className="flex flex-col items-center justify-center py-24 text-center space-y-4">
      ...
      <p className="text-sm font-medium text-muted-foreground italic">Your AI Advisor is synchronizing with your financial history...</p>
    </div>
  );

  const getUrgencyStyles = (urgency: string) => {
    switch (urgency) {
      case 'critical': return "bg-rose-50 border-rose-100 text-rose-900 dark:bg-rose-950/20 dark:border-rose-900/50 dark:text-rose-200";
      case 'warning': return "bg-amber-50 border-amber-100 text-amber-900 dark:bg-amber-950/20 dark:border-amber-900/50 dark:text-amber-200";
      default: return "bg-blue-50 border-blue-100 text-blue-900 dark:bg-blue-950/20 dark:border-blue-900/50 dark:text-blue-200";
    }
  };
  ...
  const getAlertIcon = (type: string) => {
    switch (type) {
      ...
      case 'spike': return <TrendingDown className="h-4 w-4" />;
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
           <div className="space-y-1">
             ...
             <p className="text-muted-foreground text-xs font-medium">Suggestions based on your last 6 months of habits</p>
           </div>
           <Button variant="outline" size="sm" className="h-8 text-xs font-bold rounded-lg px-4 bg-primary text-primary-foreground border-none hover:bg-primary/90">
             Apply All Changes
           </Button>
        </div>
            ...
            <Card key={i} className="border-none shadow-sm hover:ring-2 hover:ring-primary/10 transition-all border-l-4 border-l-indigo-500">
               <CardContent className="p-6 flex gap-6">
                  <div className="flex-1 space-y-4">
                     <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-[10px] text-muted-foreground font-bold italic">Avg spend: {formatVND(s.currentAverage)}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[10px] uppercase font-black text-muted-foreground tracking-tighter">Proposed Limit</p>
                  ...
                  <div className="flex flex-col gap-2 shrink-0">
                     <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-4">
           <h3 className="text-[10px] font-black uppercase text-muted-foreground tracking-widest px-1">Goal Integration</h3>
