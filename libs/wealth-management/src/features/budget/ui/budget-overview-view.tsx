...
import { Card, CardContent, CardHeader, CardTitle, Progress, Button, CategoryBadge, MaskedBalance } from "@wealth-management/ui";
           ...
           <>
            <Card className="border-none bg-zinc-50 dark:bg-zinc-900 shadow-sm">
               <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                     <p className="text-[10px] font-black uppercase text-muted-foreground tracking-tighter">Forecasted Savings</p>
                     <p className="text-sm font-bold text-zinc-800 dark:text-zinc-200"><MaskedBalance amount={insights.forecastedSavings} /></p>
                  </div>
               </CardContent>
            </Card>
            <Card className="border-none bg-zinc-50 dark:bg-zinc-900 shadow-sm">
               <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                     <p className="text-[10px] font-black uppercase text-muted-foreground tracking-tighter">Unusual Velocity</p>
      ...
      {isLoadingAdvisor ? (
        ...
        <div className="bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/50 rounded-xl p-5 flex gap-4">
           ...
           <div className="space-y-1">
              <p className="text-[10px] font-black uppercase text-blue-600/60 tracking-wider">Advisor's Executive Summary</p>
      ...
      <Card className="border-none shadow-sm overflow-hidden relative">
        <CardContent className="p-6 space-y-4">
          <div className="flex justify-between items-end">
            <div className="space-y-1">
              <span className="text-[10px] font-black uppercase tracking-tighter text-muted-foreground">Master Budget Progress</span>
              <div className="text-3xl font-black tabular-nums flex items-baseline gap-2">
                {Math.round(totalProgress)}%
                {totalProgress > 100 && <span className="text-[10px] text-rose-500 font-black animate-pulse uppercase">Budget Burst</span>}
              </div>
            </div>
            <div className="text-right">
               <div className="text-sm font-bold"><MaskedBalance amount={totalSpent} /></div>
               <div className="text-[10px] text-muted-foreground font-medium">of <MaskedBalance amount={totalLimit} /> budget</div>
            </div>
          </div>
          
          <div className="space-y-2">
             <Progress value={Math.min(totalProgress, 100)} indicatorColor={getProgressColor(totalProgress)} className="h-3 bg-zinc-100 dark:bg-zinc-800" />
             ...
             <div className="flex items-center justify-between text-[10px] font-bold">
                <div className="flex items-center gap-1.5 text-zinc-500">
                   ...
                   <span>Daily Burn: <span className="text-zinc-900 dark:text-zinc-100"><MaskedBalance amount={dailyBurnRate} /></span> / day</span>
                </div>
                     ...
                     >
                        <div className="flex justify-between items-center">
                           ...
                           <div className="flex gap-8 tabular-nums font-bold text-sm">
                              <div className="w-24 text-right"><MaskedBalance amount={item.monthlySpent} /></div>
                              <div className="w-24 text-right text-muted-foreground font-medium"><MaskedBalance amount={item.monthlyLimit} /></div>
                           </div>
                        </div>
                        <div className="flex items-center gap-4">
                           <Progress value={Math.min(progress, 100)} indicatorColor={getProgressColor(progress)} className="h-1 flex-1 bg-zinc-100 dark:bg-zinc-800" />
                           <span className="text-[10px] font-black text-muted-foreground w-8 text-right italic">{Math.round(progress)}%</span>
                        </div>
                     </div>
                  );
               })}
