...
export async function getGoalProjection(id: string): Promise<GoalProjection | null> {
  ...
  const daysRemaining = Math.max(0, (new Date(goal.deadline).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
  ...
  return {
    currentPace: {
      ...
      monthlyContribution: goal.history.length > 0 ? goal.currentAmount / (goal.history.length / 12) : 0,
    },
    requiredPace: {
      completionDate: goal.deadline,
