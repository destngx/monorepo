...
import { getLoans } from '@/lib/sheets/loans';
...
import { type DataContext } from '@/lib/ai/prompts/market/investment';
...
export async function extractInvestmentData(accounts: any[]): Promise<DataContext> {
  ...
  const accountsSummary = accounts.map((a: any) => ({
    name: a.name, balance: a.balance, currency: a.currency, type: a.type
  }));
