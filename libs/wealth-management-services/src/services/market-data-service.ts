 ...
 */
export async function getMarketPulseData(timeframe: '1h' | '4h' | '1d' | '1w' = '1h', forceRefresh: boolean = false): Promise<MarketPulseResponse> {
    ...
    if (vnGoldRes.ok) {
      ...
      if (vnGoldData.success) {
        ...
        vnData.assets.push({
          ...
          closes: usGold?.closes ? usGold.closes.map((c, i) => c * (realCloses && realCloses[i] ? realCloses[i] : 25400)) : []
        });
        vnGoldAdded = true;
      }
    }
...
async function generateAiMarketAnalysis(us: MarketState, vn: MarketState, timeframe: string) {
  ...
  const prompt = `
    ...
    - Key Support/Resistance: ${us.technicals?.supportResistance.slice(0, 3).map(sr => `${sr.symbol}: S:[${sr.support.join(',')}] R:[${sr.resistance.join(',')}]`).join(' | ')}
    ...
    - Key Support/Resistance: ${vn.technicals?.supportResistance.slice(0, 3).map(sr => `${sr.symbol}: S:[${sr.support.join(',')}] R:[${sr.resistance.join(',')}]`).join(' | ')}
 ...
 */
async function fetchMarketGroup(tickers: { symbol: string; name: string }[], market: 'US' | 'VN', timeframe: string): Promise<MarketState> {
  ...
  if (market === 'VN' && timeframe === '1h') {
    ...
    const [vnIndices, fxData] = await Promise.all([
      fetchVNIndicesFromCafeF(),
    ...
    const indexMap: Record<string, number> = {
      'VN-Index': 1, 'HNX': 2, 'VN30': 11, 'UPCOM': 9
    };
 ...
 */
async function fetchAssetData(symbol: string, name: string, market: 'US' | 'VN', timeframe: string = '1h'): Promise<MarketAsset | null> {
  try {
    ...
    const weekChange = closes[oneWeekAgoIdx] ? ((currentPrice - closes[oneWeekAgoIdx]) / closes[oneWeekAgoIdx]) * 100 : 0;
...
function generateValuation(assets: MarketAsset[], market: 'US' | 'VN'): Valuation {
  ...
  const valuationAssets = assets.filter(a => 
  ...
  const dcf = valuationAssets.map(a => {
    ...
    return {
      ...
      assumptions: [
        ...
        { label: 'Mean', value: fairValue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) }
      ]
    };
  });
