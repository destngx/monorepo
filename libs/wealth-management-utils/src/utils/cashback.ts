...
export function getCreditCardSummary(transactions: Transaction[], accounts?: any[]) {
  ...
  const sharedAccount = accounts?.find(a =>
    a.name === 'Credit Card Sacombank' && a.type?.toLowerCase().includes('negative')
  );

  // First, calculate individual card stats
  const cardResults = knownCards.map(cardInfo => {
    // Find specific account or use shared
    const realAccount = accounts?.find(a =>
      a.name.match(cardInfo.pattern) && a.type?.toLowerCase().includes('negative')
    ) || sharedAccount;

    const accountKey = realAccount?.name || (cardInfo.tagKeyword === 'uniq' ? 'Credit Card Sacombank UNIQ' : 'Credit Card Sacombank');
