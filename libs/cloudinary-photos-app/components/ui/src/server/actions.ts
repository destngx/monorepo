...
export async function addImageToAlbum(image: SearchResult, album: string) {
  await (cloudinary as any).v2.api.create_folder(album);
  ...
  await (cloudinary as any).v2.uploader.rename(image.public_id, `${album}/${publicId}`);
}

export async function setAsFavoriteAction(publicId: string, isFavorite: boolean) {
  if (isFavorite) {
    await (cloudinary as any).v2.uploader.add_tag('favorite', [publicId]);
  } else {
    await (cloudinary as any).v2.uploader.remove_tag('favorite', [publicId]);
  }
}

export async function getFolders() {
  const { folders } = (await (cloudinary as any).v2.api.root_folders()) as {
    folders: Folder[];
  };
